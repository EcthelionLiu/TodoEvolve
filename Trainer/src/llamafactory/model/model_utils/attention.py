...
def configure_attn_implementation(config: "PretrainedConfig", model_args: "ModelArguments") -> None:
    ...
    if getattr(config, "model_type", None) == "gpt_oss":
        ...
        # [FORCE FIX] 强制使用标准的 Flash Attention 2，跳过 FA3 内核检测
        requested_attn_implementation = "flash_attention_2"
        setattr(config, "_attn_implementation", requested_attn_implementation)
        setattr(config, "_attn_implementation_internal", requested_attn_implementation)
        model_args.flash_attn = AttentionFunction.FA2

        logger.info_rank0("Force using Standard FlashAttention-2 for gpt-oss model (Manual Patch).")
        return
